...
unsigned int nStakeMinAge = 24 * 60 * 60 * 15; //Minimum stake age 15days
    unsigned int nStakeMaxAge = 24 * 60 * 60 * 30; //Maximum stake age 90days
...
const string strMessageMagic = "samaritan Signed Message:\n";
...
int BLOCKS_PER_DAY = 720;
int DIVISION_PERIOD_DAYS = 140;

// miner's coin base reward
int64_t GetProofOfWorkReward(int64_t nFees)
{
    
            int64_t nSubsidy = 200 * COIN; //Miner reward

            if(nBestHeight == 0)
            {
                nSubsidy = 20000000 * COIN; //Total premine coin
            }else if(nBestHeight > LAST_POW_BLOCK){
                nSubsidy = 0;
            }else{
                nSubsidy = nSubsidy >> (nBestHeight / (BLOCKS_PER_DAY*DIVISION_PERIOD_DAYS));
            }
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (vtx.empty() || !vtx[0].IsCoinBase())
        return DoS(100, error("CheckBlock() : no transaction in block", vtx.empty()));
    if(!vtx[0].IsCoinBase()){
        return DoS(100, error("CheckBlock() : first tx is not coinbase\n%s", vtx[0].ToString().c_str()));
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "samaritan", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Maharadja 19 October 2017 Programs Final Meeting";
        CTransaction txNew;
        ...
        block.nNonce   = !fTestNet ? 401760 : 401760;
        ...
        assert(block.hashMerkleRoot == uint256("0x6f11b0b8aa66cd591842bd707e5fbf9b6888386e345637a4589a1940b838b537"));
...
unsigned char pchMessageStart[4] = { 0xf3, 0x5d, 0xa3, 0x6f };
